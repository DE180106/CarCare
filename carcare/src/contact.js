...
import { Navbar, Container, Nav, Button, Row, Col, Form, InputGroup, Card } from 'react-bootstrap';
import { Link, useParams } from 'react-router-dom';

function Contact() {
  const { userId, garageId } = useParams(); // Lấy từ URL params
  ...
  useEffect(() => {
    if (!userId || !garageId) {
      setError('Vui lòng cung cấp userId và garageId');
      setLoading(false);
      return;
    }
  ...
  const sendMessage = async () => {
    ...
    try {
      let updatedThread;
      if (threadId) {
        const response = await fetch(`http://localhost:9999/ChatThreads/${threadId}`);
        if (!response.ok) {
        ...
        const updateResponse = await fetch(`http://localhost:9999/ChatThreads/${threadId}`, {
          method: 'PUT',
        ...
        updatedThread = {
          id: `c${Date.now()}`,
          participants: { userId, garageId },
          messages: [newMsg],
        };

        const createResponse = await fetch('http://localhost:9999/ChatThreads', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(updatedThread),
        });
  ...
  return (
    <>
      ...
      <Container className="my-5">
        <Card>
          <Card.Header as="h4">Chat với Garage {garageId} (User {userId})</Card.Header> {/* Thêm để hiển thị rõ */}
          <Card.Body>
            <div
              style={{
                height: '400px',
            ...
            >
              {messages.length === 0 ? (
                <p className="text-muted text-center">Chưa có tin nhắn nào. Bắt đầu chat ngay!</p>
              ) : (
                messages.map((msg, index) => (
                  ...
                  >
                    ...
                    >
                      <strong>{msg.sender === 'user' ? 'Bạn' : 'Garage'}: </strong>
                      {msg.message}
                    </div>
