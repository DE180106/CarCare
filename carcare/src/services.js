...
const BookingForm = () => {
  ...
  // Fetch dữ liệu ban đầu từ Users và Garages
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!usersResponse.ok) {
          console.error('Lỗi khi lấy Users:', usersResponse.status, usersResponse.statusText);
          throw new Error(`Lỗi khi lấy dữ liệu Users: ${usersResponse.status} ${usersResponse.statusText}`);
        }
        ...
        if (!garagesResponse.ok) {
          console.error('Lỗi khi lấy Garages:', garagesResponse.status, garagesResponse.statusText);
          throw new Error(`Lỗi khi lấy dữ liệu Garages: ${garagesResponse.status} ${garagesResponse.statusText}`);
        }
  ...
  // Tự động điền thông tin dựa trên fullName và phone
  useEffect(() => {
    const lookupUser = () => {
      if (formData.fullName.trim() && formData.phone.trim() && users.length > 0) {
        console.log('Tìm kiếm người dùng với:', formData.fullName, formData.phone);
        ...
        if (user) {
          ...
          setFormData(prev => ({
            ...prev,
            email: user.email || '',
            productModel: user.car && user.car.make && user.car.model ? `${user.car.make} ${user.car.model}` : '',
            licensePlate: user.car && user.car.licensePlate ? user.car.licensePlate : '',
          }));
          setErrors(prev => ({
            ...prev,
            email: undefined,
          ...
          setStatusMessage('Không tìm thấy thông tin người dùng. Vui lòng nhập đầy đủ thông tin thủ công.');
        }
      } else {
        setStatusMessage('');
      }
  ...
  const handleChange = (e) => {
    ...
    validateField(name, type === 'checkbox' ? (checked ? [...formData.services, value] : formData.services.filter(s => s !== value)) : value);
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      case 'fullName':
        ...
        else if (value.length > 80) newErrors.fullName = 'Họ tên không được vượt quá 80 ký tự.';
        else delete newErrors.fullName;
        break;
      case 'phone':
        if (!value.trim()) newErrors.phone = 'Số điện thoại là bắt buộc.';
        else if (!/^\d{10}$/.test(value)) newErrors.phone = 'Số điện thoại phải là 10 chữ số.';
      ...
      case 'licensePlate':
        ...
        else if (!/^[A-Za-z0-9-.]{5,10}$/.test(value)) newErrors.licensePlate = 'Biển số xe không hợp lệ (chỉ chấp nhận chữ, số, dấu gạch ngang và dấu chấm).';
        else delete newErrors.licensePlate;
        break;
      case 'services':
        if (value.length === 0) newErrors.services = 'Vui lòng chọn ít nhất một dịch vụ.';
      ...
      case 'scheduleDate':
        ...
        else if (new Date(value) < new Date(new Date().toISOString().split('T')[0])) newErrors.scheduleDate = 'Ngày phải từ hôm nay trở đi.';
  ...
  // Lọc garage dựa trên dịch vụ đã chọn
  useEffect(() => {
    if (formData.services.length > 0) {
      const filteredGarages = allGarages.filter(garage =>
        formData.services.every(service => garage.services && garage.services.includes(service))
      );
  ...
  // Lấy khung giờ khả dụng dựa trên garage và ngày
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Tìm userId từ Users dựa trên fullName và phone
      console.log('Tìm userId từ Users...');
      const user = users.find(
        u => u.fullName === formData.fullName && u.phone === formData.phone
      );
      if (!user) {
        console.log('Không tìm thấy người dùng với fullName:', formData.fullName, 'và phone:', formData.phone);
        setStatusMessage('Không tìm thấy người dùng. Vui lòng kiểm tra họ tên và số điện thoại.');
        return;
      }
      console.log('Tìm thấy userId:', user.id);

      // Lấy ID booking cuối cùng để tạo ID mới
      ...
      if (!bookingsResponse.ok) {
        console.error('Lỗi khi lấy Bookings:', bookingsResponse.status, bookingsResponse.statusText);
        throw new Error(`Lỗi khi lấy dữ liệu Bookings: ${bookingsResponse.status} ${bookingsResponse.statusText}`);
      }
      ...
      if (isDuplicate) {
        console.log('Tìm thấy lịch trùng:', formData.garageId, formData.scheduleDate, formData.scheduleTime);
      ...
      // Gửi POST request đến http://localhost:9999/Bookings
      ...
      if (!response.ok) {
        console.error('Lỗi khi tạo booking:', response.status, response.statusText);
        throw new Error(`Lỗi khi tạo booking: ${response.status} ${response.statusText}`);
      }
      ...
      if (window.bootstrap && window.bootstrap.Modal) {
        const modal = new window.bootstrap.Modal(document.getElementById('schedule-maintenance-popup'));
        modal.show();
      } else {
      ...
      <div className="dat-lich-bao-duong py-5">
        <Container>
          ...
          {statusMessage && (
            <div
              className={`alert ${
                statusMessage.includes('thành công') ? 'alert-success' : 'alert-danger'
              } text-center mb-4`}
              role="alert"
            >
              {statusMessage}
          ...
          <form className="schedule-maintenance-form" onSubmit={handleSubmit}>
            <Row>
              ...
              <Col lg={6} xs={12} className="mb-4">
                <h5 className="mb-3">
                  <span className="badge bg-primary me-2">1</span>Thông tin khách hàng
                </h5>
                <div className="form-group mb-3">
                  ...
                  <div className="input-group">
                    <input
                      ...
                      required
                    />
                    <span className="input-group-text">{formData.fullName.length}/80</span>
                  ...
                  <div className="border p-3 rounded">
                    {allServices.map((service, idx) => (
                      <div className="form-check" key={idx}>
                        ...
                        />
                        <label className="form-check-label" htmlFor={`service_${idx}`}>
                          {service}
                        </label>
                      </div>
                    ))}
                    {errors.services && <div className="text-danger mt-2">{errors.services}</div>}
                  </div>
                </div>
                <div className="form-group mb-3">
              ...
              <Col lg={6} xs={12} className="mb-4">
                <h5 className="mb-3">
                  <span className="badge bg-primary me-2">4</span>Địa điểm và Thời gian
                </h5>
                <div className="form-group mb-3">
                  ...
                  {errors.garageId && <div className="invalid-feedback">{errors.garageId}</div>}
                </div>
                <div className="form-group mb-3">
                  <label>
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                <div className="modal-body text-center">
                  ...
                  ></button>
                  <div className="mt-3">
                    <i className="bi bi-check-circle text-success fa-3x"></i>
                  </div>
                  <h4 className="mt-3">Tiếp nhận đặt lịch</h4>
                  <p>
                    Gara đã tiếp nhận thông tin đặt hẹn của bạn. Chúng tôi sẽ xử lý và phản hồi lại để xác nhận lịch hẹn trong thời gian sớm nhất.
                  </p>
                  <button
                    type="button"
