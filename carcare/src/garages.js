...
const StoreSystem = () => {
  ...
  useEffect(() => {
    if (!loading && garages.length > 0 && !mapInstanceRef.current && mapRef.current) {
      const avgLat = garages.reduce((sum, g) => sum + parseFloat(g.latitude), 0) / garages.length;
      const avgLng = garages.reduce((sum, g) => sum + parseFloat(g.longitude), 0) / garages.length;
      ...
      garages.forEach(garage => {
        if (garage.latitude && garage.longitude) {
          L.marker([parseFloat(garage.latitude), parseFloat(garage.longitude)], {
            icon: L.icon({
    ...
    <div>
        ...
        <Container>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="ms-auto">
              ...
              <Nav.Link as={Link} to="/services" active={location.pathname === '/services'}>Dịch vụ</Nav.Link>
              <Nav.Link as={Link} to="/garages" active={location.pathname === '/garages'}>Garage</Nav.Link>
              <Nav.Link as={Link} to="/about" active={location.pathname === '/about'}>Giới thiệu</Nav.Link>
              <Nav.Link as={Link} to="/contact" active={location.pathname === '/contact'}>Liên hệ</Nav.Link>
      ...
      <div className="min-h-screen bg-gray-50 p-4 md:p-6">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-2xl font-bold text-center mb-4">Hệ Thống Cửa Hàng</h1>
          ...
          <div className="row g-4">
            ...
            <div className="col-md-4 bg-white rounded shadow p-4">
              ...
              >
                ...
                {garages.filter(g => g.status === 'open').map(g => (
              ...
              {selectedShop && (
                <div className="mt-4 text-sm">
                  ...
                  {selectedShop.phone && <p><strong>Điện thoại:</strong> {selectedShop.phone}</p>}
                </div>
              )}
            </div>
