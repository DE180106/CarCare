...
const AdminDashboard = () => {
  ...
  // Lấy dữ liệu từ API hoặc dữ liệu dự phòng
  useEffect(() => {
    ...
    const fetchGarages = async () => {
      try {
        console.log('Đang lấy danh sách garage từ API...');
        ...
        if (fetchedGarages.length === 0) {
          ...
          setError(prev => prev ? `${prev} Không tìm thấy garage trong API.` : 'Không tìm thấy garage trong API.');
        } else {
        ...
        setGarages(data.Garages);
        setError(prev => prev ? `${prev} Lỗi khi lấy danh sách garage: ${err.message}.` : `Lỗi khi lấy danh sách garage: ${err.message}.`);
      }
    };

    const fetchBookings = async () => {
      try {
        console.log('Đang lấy danh sách đặt lịch từ API...');
        const fetchedBookings = await fetchWithRetry('http://localhost:9999/Bookings', {
          headers: { 'Content-Type': 'application/json' },
          signal: AbortSignal.timeout(10000),
        });
        ...
        if (fetchedBookings.length === 0) {
          console.warn('Không tìm thấy đặt lịch trong API. Sử dụng dữ liệu dự phòng.');
          setBookings(data.Bookings);
          setError(prev => prev ? `${prev} Không tìm thấy đặt lịch trong API.` : 'Không tìm thấy đặt lịch trong API.');
        } else {
          setBookings(fetchedBookings);
        }
      } catch (err) {
        console.error('Lỗi khi lấy đặt lịch:', err);
        setBookings(data.Bookings);
        setError(prev => prev ? `${prev} Lỗi khi lấy danh sách đặt lịch: ${err.message}.` : `Lỗi khi lấy danh sách đặt lịch: ${err.message}.`);
      }
    };

    const fetchFeedbacks = async () => {
      try {
        console.log('Đang lấy danh sách feedback từ API...');
        const fetchedFeedbacks = await fetchWithRetry('http://localhost:9999/Feedback', {
          headers: { 'Content-Type': 'application/json' },
          signal: AbortSignal.timeout(10000),
        });
        ...
        if (fetchedFeedbacks.length === 0) {
          console.warn('Không tìm thấy feedback trong API. Sử dụng dữ liệu dự phòng.');
          setFeedbacks(data.Feedback);
          setError(prev => prev ? `${prev} Không tìm thấy feedback trong API.` : 'Không tìm thấy feedback trong API.');
        } else {
        ...
        setFeedbacks(data.Feedback);
        setError(prev => prev ? `${prev} Lỗi khi lấy danh sách feedback: ${err.message}.` : `Lỗi khi lấy danh sách feedback: ${err.message}.`);
      }
    };

    const fetchRepairDetails = async () => {
      try {
        console.log('Đang lấy danh sách sửa chữa từ API...');
        const fetchedRepairDetails = await fetchWithRetry('http://localhost:9999/GarageDashboard/RepairDetails', {
          headers: { 'Content-Type': 'application/json' },
          signal: AbortSignal.timeout(10000),
        });
        if (!Array.isArray(fetchedRepairDetails)) {
          throw new Error('Dữ liệu API không phải là mảng');
        }
        if (fetchedRepairDetails.length === 0) {
          console.warn('Không tìm thấy lịch sử sửa chữa trong API. Sử dụng dữ liệu dự phòng.');
          setRepairDetails(data.GarageDashboard.RepairDetails);
          setError(prev => prev ? `${prev} Không tìm thấy lịch sử sửa chữa trong API.` : 'Không tìm thấy lịch sử sửa chữa trong API.');
        } else {
          setRepairDetails(fetchedRepairDetails);
        }
      } catch (err) {
        console.error('Lỗi khi lấy lịch sử sửa chữa:', err);
        setRepairDetails(data.GarageDashboard.RepairDetails);
        setError(prev => prev ? `${prev} Lỗi khi lấy lịch sử sửa chữa: ${err.message}.` : `Lỗi khi lấy lịch sử sửa chữa: ${err.message}.`);
      }
    };

    const fetchData = async () => {
      setLoading(true);
      await Promise.all([fetchUsers(), fetchGarages(), fetchBookings(), fetchFeedbacks(), fetchRepairDetails()]);
      setLoading(false);
    };
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Không thể xóa người dùng: ${response.status}`);
  ...
  const handleDeleteGarage = async (garageId) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Không thể xóa garage: ${response.status}`);
  ...
  const handleDeleteBooking = async (bookingId) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:9999/Bookings/${bookingId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
      if (!response.ok) throw new Error(`Không thể xóa đặt lịch: ${response.status}`);
  ...
  const handleDeleteFeedback = async (feedbackId) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Không thể xóa feedback: ${response.status}`);
